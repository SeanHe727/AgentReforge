...
Orient -> Diagnose symptom/root cause/capability gap
       ...
       -> Rank and select by evidence/benefit/risk/effort
...
1. A deterministic contract runner executes the frozen full-candidate commands
   and evaluates exit codes and declared output assertions.
2. The Deliverer LLM receives the frozen Orchestrator goals, selected causal
   mechanism, high-level requirements, and the complete loop diff. It decides
   whether the coherent change actually realizes that proposal.
